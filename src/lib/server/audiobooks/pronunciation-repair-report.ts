...
export async function pronunciationRepairReport(bookId: string, userId: string, jobId: string) {
  ...
  return {
    ...
    requestedModel: settings.aiModel, fallbackModels: settings.fallbackModels, profileId: settings.profileId, error: job.error,
    nextAttemptAt: job.status === 'queued' ? settings.nextAttemptAt : undefined,
    retryScheduleActive: job.status === 'queued' && Boolean(settings.nextAttemptAt),
    retryTimingNote: 'Only the top-level nextAttemptAt on a queued job is an active retry schedule. Timestamps inside chapter diagnostics and previousAttempts are historical retry suggestions, not scheduled work.',
