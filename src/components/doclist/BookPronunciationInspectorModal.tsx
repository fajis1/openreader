...
export function BookPronunciationInspectorModal({
    ...
    try {
      ...
      toast.promise(
        fetch('/api/audiobooks/batch-replace', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ word, newPhonetic: phonetic })
        }).then(async res => {
  ...
  const handleSaveDefinition = async (word: string, newDef: string) => {
    try {
      const res = await fetch('/api/tts/global-definitions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ word, definition: newDef })
      });
      if (!res.ok) throw new Error('Failed to update definition');
      
      const data = await res.json();
      setWords(prevWords => prevWords.map(w => w.word === word ? { ...w, definition: data.definition } : w));
      setEditingDefinition(null);
      toast.success(data.definition ? `Updated definition for ${word}` : `Removed definition for ${word}`);
    } catch (e) {
      console.error(e);
      ...
      if (res.ok) {
        ...
        if (data.newChoices && data.newChoices.length > 0) {
          setWords(prev => prev.map(w => {
            if (w.word === word) {
              const existing = w.globalChoices || [];
              const uniqueNew = data.newChoices.filter((c: string) => !existing.includes(c));
              return { ...w, globalChoices: [...uniqueNew, ...existing] };
            }
            return w;
          }));
        }
        ...
        if (res.status === 401 || res.status === 400 || errMsg.toLowerCase().includes('api key')) {
  ...
  const handleSaveNew = async () => {
    ...
    try {
      ...
      const globalPayload: any = { word: newWord.trim(), phonetic };
      if (isAdmin) {
  ...
  const filterCache = React.useRef(new Map<string, boolean>());
  ...
  const filteredWords = React.useMemo(() => {
    return words.filter(w => {
      ...
      let matched = filterCache.current.get(w.word);
      if (matched === undefined) {
         const isMatch = useFuzzySearch ? isSimilar(w.word, searchQuery) : w.word.toLowerCase().includes(searchQuery.toLowerCase());
         filterCache.current.set(w.word, isMatch);
         return isMatch;
      }
      return matched;
    });
  ...
  return (
    <ModalFrame 
      ...
      onClose={onClose} 
      size="xl" 
      panelClassName={isShiftedRight ? "transition-transform duration-300 translate-x-[25vw] max-w-[50vw]" : "transition-transform duration-300 !max-w-7xl"}
    >
      <div className="flex flex-col h-[85vh] bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 relative">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-sm font-semibold">
                  <th className="p-3 w-1/5">Word</th>
                  <th className="p-3 w-1/5">Global Choices</th>
                  <th className="p-3 w-1/5">My Active Profile</th>
                  <th className="p-3 w-1/5">AI Generator</th>
                  <th className="p-3 w-1/5">Definition</th>
                </tr>
              </thead>
              <tbody>
                {renderedWords.map((w, idx) => (
                  <tr key={idx} className="border-b dark:border-gray-700">
                    <td className="p-3 align-top max-w-[200px]">
                      <div className="font-bold text-lg [overflow-wrap:anywhere]">{w.word}</div>
                    ...
                    <td className="p-3 align-top">
                      <div className="flex flex-col gap-2">
                        {(() => {
                          const choicesToRender = [...(w.globalChoices || [])];
                          if (w.globalDefault && !choicesToRender.includes(w.globalDefault)) {
                            choicesToRender.unshift(w.globalDefault);
                          }
                          return choicesToRender.map((choice: string, cIdx: number) => (
                    ...
                    <td className="p-3 align-top min-w-[180px]">
                      <div className="flex flex-col gap-2">
                        <button
                          className="px-3 py-1.5 text-xs font-semibold bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300 rounded border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-200 dark:hover:bg-indigo-800 text-center flex items-center justify-center gap-1 transition-colors"
                          onClick={() => {
                      ...
                      {editingDefinition === w.word ? (
                        <div className="flex flex-col gap-1">
                          <textarea 
                            ...
                            onChange={e => setDefinitionEditValue(e.target.value)}
                            placeholder="Blank omits this definition"
                            className="w-full p-1 text-xs border rounded bg-white dark:bg-gray-900 dark:border-gray-700 min-h-[60px]"
                            autoFocus
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && !e.shiftKey) {
                                e.preventDefault();
                                handleSaveDefinition(w.word, definitionEditValue);
                              }
                              if (e.key === 'Escape') setEditingDefinition(null);
                            }}
                          />
                          <div className="flex gap-1 mt-1">
                            <button className="px-2 py-1 text-[10px] font-semibold bg-blue-600 text-white rounded hover:bg-blue-700" onClick={() => handleSaveDefinition(w.word, definitionEditValue)}>Save</button>
                            <button className="px-2 py-1 text-[10px] font-semibold bg-gray-200 text-gray-800 rounded hover:bg-gray-300" onClick={() => setEditingDefinition(null)}>Cancel</button>
                          </div>
                        </div>
                      ) : w.definition ? (
                        <div>
                        ...
                        <div>
                          <span className="text-xs italic text-gray-500">No definition</span>
                          <button
                            type="button"
                            onClick={() => {
                              setEditingDefinition(w.word);
